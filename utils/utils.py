...
@torch.no_grad()
def create_memory_bank(model, data_loader, device, num_cores=None, labels=None):
    model.eval()
    embeddings = []
    indices = []
...
# Source: https://github.com/decisionintelligence/CATCH/blob/master/ts_benchmark/baselines/catch/layers/RevIN.py
class RevIN1d(nn.Module):
    def __init__(self, num_channels: int, eps: float = 1e-5,
                 min_sigma: float = 1e-5, affine: bool = False):
        super().__init__()
        self.eps = eps
